...
async def handle_invites(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if "acc" in data:
        if data.startswith("guess"):
            ...
            game_obj = {"op": challenger, "number": num, "turn": challenger, "tries": 5}
            # مزامنة نفس الكائن لكلا اللاعبين
            active_guess_games[uid] = active_guess_games[challenger] = game_obj
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if uid in active_guess_games:
        ...
        if g == game["number"]:
            ...
            await context.bot.send_message(op, "🏆 فزت! الخصم استنفد جميع محاولاته.")
            del active_guess_games[uid], active_guess_games[op]
        else:
            hint = "أصغر 📉" if g > game["number"] else "أكبر 📈"
            # تبديل الدور في كلا السجلين
            game["turn"] = op
